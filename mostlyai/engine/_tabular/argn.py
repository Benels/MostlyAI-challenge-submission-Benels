...
def _embedding_heuristic(id: str, model_size: ModelSizeOrUnits, dim_input: int) -> int:
    ...
    model_size_output_dim = dict(
        ...
        M=max(18, int(4 * np.ceil(dim_input**0.39516983567152547))),
        L=max(17, int(4 * np.ceil(dim_input**0.3456641218404693))),
    )
...
def _regressor_heuristic(id: str, model_size: ModelSizeOrUnits, dim_input: int, cardinality: int) -> list[int]:
    ...
    for idx, unit in enumerate(layers[:-1]):
        #coefficient = max(1, round(np.log(max(cardinality, np.e))))
...
class HistoryCompressor(nn.Module):
    def __init__(
        ...
        self.device = device
        dropout_rate = 0.30253084163987004
...
class Regressors(nn.Module):
    def __init__(
        ...
        *,
        ...
        if model_size == "L":
            self.dropout_rate = 0.04818078201461677
        else:
            self.dropout_rate = 0.05456876702827298
