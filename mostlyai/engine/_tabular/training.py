    ...
    *,
    ...
    with ProgressCallbackWrapper(
        ...
        if is_sequential:
            _what = "sequential"
            _factor = 0.2
            _decay = np.float64(8.020130891025759e-05)
            _initial_learn_rate = initial_lr
        else:
            _what = "flat"
            _factor = 0.1
            _decay = np.float64(0.003895813805322184)
            _initial_learn_rate = np.float64(0.000200298699117235)

        _LOG.info(f"training {model} model in {_what} mode with {_factor=} and {_decay=} and {_initial_learn_rate=}")


        early_stopper = EarlyStopper(val_loss_patience=10)
        optimizer = torch.optim.AdamW(params=argn.parameters(), lr=_initial_learn_rate, weight_decay=_decay)
